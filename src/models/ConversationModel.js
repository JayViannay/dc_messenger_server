...
const getMessages = (id) => {
  return new Promise((resolve, reject) => {
    db.query(
      "SELECT * FROM message WHERE conversation_id = ? ORDER BY created_at DESC",
      id,
      (err, results) => {
        if (err) reject(err);
...
const getParticipants = (id) => {
  return new Promise((resolve, reject) => {
    db.query(
      "SELECT user.id, user.email FROM conversation_user INNER JOIN user ON conversation_user.user_id = user.id WHERE conversation_id = ?",
      id,
      (err, results) => {
        if (err) reject(err);
        else resolve(results);
